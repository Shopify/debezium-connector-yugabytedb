...
public class YugabyteDBSchemaHistoryProducer {
    ...
    // Singleton instances - one producer per connector (shared across all tasks)
    ...
    /**
     * Gets or creates a schema history producer for the given connector.
     * Uses singleton pattern to ensure one producer per connector across all tasks.
     *
     * @param topicName the Kafka topic to write schema history to
     * @param bootstrapServers the Kafka bootstrap servers
     * @param connectorName the connector name (used as key prefix and singleton key)
     ...
     * @return the singleton producer instance for this connector
     */
    public static YugabyteDBSchemaHistoryProducer getInstance(
        ...
        return INSTANCES.computeIfAbsent(connectorName, key -> {
            LOGGER.info("Creating singleton schema history producer for connector: {}", connectorName);
            REFERENCE_COUNTS.put(connectorName, new AtomicInteger(0));
     ...
     */
    public void acquire() {
        AtomicInteger refCount = REFERENCE_COUNTS.get(connectorName);
        if (refCount != null) {
            int count = refCount.incrementAndGet();
            LOGGER.debug("Schema history producer for {} acquired, ref count: {}", connectorName, count);
        }
    }
    ...
    public void release() {
        ...
        if (refCount != null) {
            ...
            LOGGER.debug("Schema history producer for {} released, ref count: {}", connectorName, count);

            if (count <= 0) {
                LOGGER.info("Schema history producer for {} has no more references, closing", connectorName);
                YugabyteDBSchemaHistoryProducer instance = INSTANCES.remove(connectorName);
                REFERENCE_COUNTS.remove(connectorName);
                if (instance != null) {
